...
Source: [`useless_OS_clean.asm`](useless_OS_clean.asm)  
ROM image: [`useless_OS.hex`](useless_OS.hex)
...
```bash
python3 cpu_sim.py                    # load useless_OS.hex, interactive shell
python3 cpu_sim.py <rom.hex>          # load a specific ROM image
python3 cpu_sim.py --trace            # print every instruction to stderr
...
```bash
python3 assembler.py <source.asm>               # → <source.hex>
python3 assembler.py <source.asm> -o out.hex    # explicit output path
```
...
```bash
python3 disassembler.py                          # disassemble useless_OS.hex → useless_OS.asm
python3 disassembler.py <input.hex>              # → <input.asm>
python3 disassembler.py <input.hex> -o out.asm  # explicit output path
```
...
python3 extract_netlist.py CPU_design.circ           # explicit input
...
A cycle-accurate RTL model of the CPU in `verilog/cpu.v`.
...
```bash
cd verilog
iverilog -g2012 -Wall -o sim tb_cpu.v cpu.v
./sim
```
...
**ROM init file (`rom_init.mem`):** Generated from `useless_OS.hex` by the Makefile. One 12-bit hex value per line, 256 entries, in `$readmemh` format.
...
homemade-CPU/
│
├── CPU_design.circ          Logisim circuit file (complete CPU + TTY + keyboard)
├── CPU_schematic.png        Block-level schematic screenshot
│
├── useless_OS.asm           Disassembled ROM (address-labelled, generated)
├── useless_OS_clean.asm     Hand-written assembly source (labels, comments)
├── useless_OS.hex           ROM image in Logisim v2.0 raw format
├── useless_OS_clean.hex     ROM image assembled from useless_OS_clean.asm
│
├── assembler.py             Two-pass assembler (.asm → Logisim .hex)
├── disassembler.py          Disassembler (Logisim .hex → .asm)
├── extract_netlist.py       Logisim .circ → netlistsvg JSON
├── cpu_sim.py               Interactive Python CPU simulator
│
└── verilog/
    ├── cpu.v                Verilog RTL — datapath + control (single-cycle)
    ├── tb_cpu.v             Simulation testbench (TTY capture + keyboard input)
    ├── rom_init.mem         ROM contents in $readmemh format (generated)
    └── Makefile             make sim / make wave / make clean
```
